...
public class HandgunScriptLPFP : MonoBehaviour
{
	...
	//check if long knifing
	private bool isLongKnifing;

	//check if magazine is full
	private bool isMagFull;

	//check if we are throwing a grenade
	private bool isGrenading;
	...
	private void Update()
	{
		...
		if (Input.GetKeyDown(KeyCode.Q) && !isInspecting && !isKnifing && !isLongKnifing && !isGrenading && !isReloading)
		{
			anim.Play("Knife Attack 1", 0, 0f);
		}
		//Play knife attack 2 animation when F key is pressed
		if (Input.GetKeyDown(KeyCode.F) && !isInspecting && !isKnifing && !isLongKnifing && !isGrenading && !isReloading)
		{
			anim.Play("Knife Attack 2", 0, 0f);
		}

		//Throw grenade when pressing G key
		if (Input.GetKeyDown(KeyCode.G) && !isInspecting && !isKnifing && !isLongKnifing && !isGrenading && !isReloading)
		{
	...
	private IEnumerator AutoReload()
	{
		...
		//delay so reload happens after animation
